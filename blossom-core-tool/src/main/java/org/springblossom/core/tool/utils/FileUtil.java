...
import org.springblossom.core.tool.constant.FileConstant;
...
public class FileUtil  extends org.springframework.util.FileCopyUtils {
	 ...
	 * 定义允许上传的文件扩展名
	 */
	private static HashMap<FileConstant, String> extMap = new HashMap<FileConstant, String>();
	static {
		extMap.put(FileConstant.IMAGE, ".gif,.jpg,.jpeg,.png,.bmp");
		extMap.put(FileConstant.MEDIA, ".swf,.flv,.mp3,.mp4,.wav,.wma,.wmv,.mid,.avi,.mpg,.asf,.rm,.rmvb");
		extMap.put(FileConstant.DOC, ".doc,.docx,.xls,.xlsx,.ppt,.txt,.pdf");
		extMap.put(FileConstant.FILE, ".gif,.jpg,.jpeg,.png,.bmp,.swf,.flv,.mp3,.mp4,.wav,.wma,.wmv,.mid,.avi,.mpg,.asf,.rm,.rmvb,.doc,.docx,.xls,.xlsx,.ppt,.htm,.html,.txt,.zip,.rar,.gz,.bz2");
	}
	 ...
	 * @return 返回成功与否
	 */
	public static boolean testExt(FileConstant fileConstant, String fileName) {
		String fileExt = getFileExt(fileName).toLowerCase();

		String ext = extMap.get(fileConstant).toLowerCase();
		String[] extArray = Func.toStrArray(StringPool.COMMA,ext);
		if (extArray == null || !Func.contains(extArray,fileExt)) {
			return false;
		}
		return true;
	}
	...
	public static void main(String[] args) {
		System.out.println("args = " + FileUtil.testExt(FileConstant.IMAGE,"aa.png"));
		System.out.println("args = " + FileUtil.testExt(FileConstant.IMAGE,"aa.pn"));
		System.out.println("args = " + FileUtil.testExt(FileConstant.IMAGE,"aa.pnG"));
	}
